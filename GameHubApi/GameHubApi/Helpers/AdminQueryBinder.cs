...
namespace GameHubApi.Helpers
{
    public static class AdminQueryBinder
    {
        public static AdminQueryParameters FromRequest(HttpRequest request, AdminQueryParameters? incoming = null)
        {
            ...
            if (request.Query.TryGetValue("_sort", out var s)) q.SortBy = s.ToString();
            if (request.Query.TryGetValue("_order", out var o)) q.SortOrder = o.ToString();
            if (request.Query.TryGetValue("q", out var qq)) q.Search = qq.ToString();
